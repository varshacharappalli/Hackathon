...
export const PortfolioTemplate2 = ({ data }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-gradient-to-br from-gray-900 to-gray-700 p-10 rounded-xl shadow-2xl text-white space-y-12">
      {/* Header Section */}
      ...
      {skills.length > 0 && (
        <section className="space-y-6">
          <h2 className="text-3xl font-semibold flex items-center gap-3 text-blue-300">
          ...
          <div className="flex flex-wrap gap-3">
            {skills.map((skill, index) => (
              <span
                key={index}
                className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium shadow-md"
              >
                {skill}
              </span>
            ))}
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((project, index) => (
              <div
                key={index}
                className="border border-gray-700 rounded-lg p-6 bg-gray-800 hover:shadow-xl transition-transform transform hover:-translate-y-1 duration-300"
              >
                ...
                <a
                  ...
                  className="text-blue-500 hover:text-blue-300 font-medium transition-colors duration-300"
                >
                  View Project →
                </a>
              </div>
      ...
      <footer className="border-t border-gray-700 pt-8 text-center space-y-4">
        <p className="flex items-center justify-center gap-3 text-gray-300">
          ...
          <a href={`mailto:${email}`} className="text-blue-500 hover:text-blue-300 text-lg font-medium transition-colors duration-300">
            {email}
          </a>
        </p>
